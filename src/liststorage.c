...
static int _liststorage_resize(t_liststorage *x, int size)
{
  ...
  if (size < 0) {
    pd_error(
        x, "[liststorage]: refusing to resize for negative amount of slots");
    return 0;
  }
...
static int _liststorage_checkslot(
    t_liststorage *x, const char *string, const int resize)
{
...
static void liststorage_bang(t_liststorage *x)
{
  ...
  int slot =
      _liststorage_checkslot(x, "attempting to read data from invalid slot", 0);
  if (slot < 0) {
    return;
  }
...
static void liststorage_add(
    t_liststorage *x, t_symbol *UNUSED(s), int ac, t_atom *av)
{
  int slot =
      _liststorage_checkslot(x, "attempting to add data to invalid slot", 1);
  if (slot < 0) {
    return;
  }
...
static void *liststorage_new(t_floatarg f)
{
  ...
  x->x_slotin =
      inlet_new(&x->x_obj, &x->x_obj.ob_pd, gensym("float"), gensym("slot"));
...
{
  liststorage_class = zexy_new("liststorage", liststorage_new, liststorage_free,
      t_liststorage, CLASS_DEFAULT, "F");
  ...
  /* clear all slots */
  zexy_addmethod(
      liststorage_class, (t_method)liststorage_clearall, "clearall", "");
  ...
  /* insert an empty slot at (before) given position */
  zexy_addmethod(
      liststorage_class, (t_method)liststorage_insert, "insert", "F");

  /* remove empty slots */
  zexy_addmethod(
      liststorage_class, (t_method)liststorage_compress, "compress", "");
