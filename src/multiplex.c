...
{
  t_class*c = class_new(gensym(name), (t_newmethod)mux_new, (t_method)mux_free, sizeof(t_mux), CLASS_NOINLET, A_GIMME, 0);
  return c;
}
static void dosetup()
{
  ...
  zclass_setup("mux");
  muxproxy_class = class_new(gensym("multiplex proxy"), 0, 0, sizeof(t_muxproxy), CLASS_PD | CLASS_NOINLET, 0);
